...
# be header lines, if they match the pattern used in _fix_headers()
...
def main():
    ...
    # call recurse to do replacements in final project location, starting at the
    ...
    for item in items:
        ...
        if os.path.isdir(abs_item):
            ...
            if item not in skip_file:
                ...
                # save lines
                with open(abs_item, 'w') as f:
                    f.writelines(lines)

        # fix path
        if item not in skip_path:
            fix_item = _fix_path(abs_item)

            # if a rename is required
            if fix_item != abs_item:

                # do the rename
                os.rename(abs_item, fix_item)
    ...
    for i in range(0, len(lines)):
        ...
        for item in LIST_HEADER:
            ...
            pattern = (
                ...
                r'(\S*)'        # 5. all a/n chars incl. periods (for file ext)
            ...
            # if it's blank or not a dunder in g_dict_settings, leave it alone
            # and keep going
...
def _fix_path(path):
    ...
    for key in g_dict_settings.keys():
        file_new = file_old.replace(key, g_dict_settings[key])

    # put new name back with the 'up to last' part
    path_new = os.path.join(dir_old, file_new)
